...
3. Cargo links the static lib and produces the binary, installing it to `PKG_INSTALL_DIR`
4. `scripts/patch-integrity.py` runs post-build: finds the `JENTRNG_INTG_TAG` in the binary, reads the embedded 32-byte HMAC key, computes HMAC-SHA3-256 over all `SHF_EXECINSTR` sections (`.init`, `.text`, `.fini`; excluding `.plt*`), and patches the 32-byte placeholder with the digest
...
**jitterentropy-library version:** Declared as `JENT_LIB_VERSION` in the Makefile. After changing the version, update `JENT_LIB_HASH` with the actual SHA-256 of the downloaded tarball.

**FFI surface** (`src/src/jent_ffi.rs`): declares `jent_entropy_init`, `jent_entropy_collector_alloc`, `jent_entropy_collector_free`, `jent_read_entropy` (entropy), plus `jent_sha3_tester`, `jent_sha3_256_init`, `jent_sha3_update`, `jent_sha3_final` (SHA-3), and the `Sha3Ctx` struct with constants `JENT_SHA3_256_SIZE_DIGEST`, `JENT_SHA3_256_SIZE_BLOCK`. SP800-90B compliance requires `jent_read_entropy` (not `_safe`) and the `JENT_FORCE_FIPS` flag.
...
Shell script package, no compilation. Runs once at boot (START=13, before network at START=20).
